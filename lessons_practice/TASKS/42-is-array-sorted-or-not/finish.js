...
const arraySortInfo = inputArra => {
	// Если хотя бы один элемент в массиве не является числом
	if (inputArra.some(el => typeof el !== 'number')) {
		return 'Некоторые элементы не являются числами'
	}

	// Если числа в массиве отсортированы по возрастанию
	if (
		inputArra.every((el, index) =>
			index > 0 ? el >= inputArra[index - 1] : true
		)
	) {
		return 'Массив отсортирован по возрастанию'
	}

	// Если числа в массиве отсортированы по убыванию
	if (
		inputArra.every((el, index) =>
			index > 0 ? el <= inputArra[index - 1] : true
		)
